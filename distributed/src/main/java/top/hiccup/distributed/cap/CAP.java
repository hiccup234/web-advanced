package top.hiccup.distributed.cap;
 ...
 *
 * 【一致性（C）】 All nodes see the same data at the same time
 * 所有数据库集群节点在同一时间点看到的数据完全一致，即所有节点能实时保持数据同步
 *
 * 【可用性（A）】 Reads and writes always succeed
 * 读写操作永远是成功的。即服务一直是可用的，即使集群一部分节点故障，集群整体还能正常响应客户端的读写请求。
 *
 * 【分区容错性（P）】 The system continues to operate despite arbitrary message loss or failure of part of the system
