...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 创建事件对象工厂
        BusiEventFactory factory = new BusiEventFactory();
        ...
        Disruptor<BusiEvent> disruptor =
                new Disruptor<BusiEvent>(factory, ringBufferSize, executor, ProducerType.SINGLE, new YieldingWaitStrategy());
        // 注册消费事件的方法
        disruptor.handleEventsWith(new BusiEventHandler());

        // 启动disruptor
        disruptor.start();

        // disruptor发布事件
        RingBuffer<BusiEvent> ringBuffer = disruptor.getRingBuffer();

        BusiEventProducer producer = new BusiEventProducer(ringBuffer);
